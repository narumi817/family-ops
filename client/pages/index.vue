                ...
                <div class="flex items-start justify-between mb-2">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <button
                      ...
                      class="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed touch-manipulation"
                      @click="handleDeleteLog(log)"
                    >
                      <svg
                        v-if="deletingLogId === log.id"
...
// 自分のログかどうか
const isOwnLog = (log: Log): boolean => {
  return authStore.user != null && log.user.id === authStore.user.id
}
...
const handleDeleteLog = async (log: Log) => {
  ...
  try {
    const { error } = await apiFetchAction(`/api/v1/logs/${log.id}`, {
      method: 'DELETE',
    })

    if (error.value) {
      deleteError.value = '削除に失敗しました。しばらく経ってからお試しください。'
      return
    }
